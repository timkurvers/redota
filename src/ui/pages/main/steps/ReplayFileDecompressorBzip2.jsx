...
const isBzip2Compressed = async (blob) => {
  const { length } = MAGIC_BZIP2;
  if (blob.size < length) {
    return false;
  }
  const magic = await blob.slice(0, length).text();
  return magic === MAGIC_BZIP2;
};

const ReplayFileDecompressorBzip2 = (props) => {
  ...
  useAsyncEffect(async () => {
    // Pass through file as-is when not dealing with Bzip2 compression
    if (!await isBzip2Compressed(file)) {
      next(file);
      return;
    }
...
export default ReplayFileDecompressorBzip2;
